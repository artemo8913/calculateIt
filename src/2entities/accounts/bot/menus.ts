...
const accountsMainMenu = new Menu<BotContext>("accountsMenu")
    .submenu("Посмотреть все", "selectAccountMenu").row()
    .submenu("Удалить счет", "deleteAccountMenu").row()
    .text("Добавить счет", (ctx) => ctx.conversation.enter("email")).row()
...
export { accountsMainMenu, selectAccountMenu, deleteAccountMenu };